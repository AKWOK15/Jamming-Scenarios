...
namespace VRAVE
{
	...
	public class CarAIControl : MonoBehaviour
	{
		...
		[SerializeField] public bool m_Driving;
		// whether the AI is currently actively driving or stopped.
		[SerializeField] private bool m_IsTargetReached;
		// doesn't stop right at the target, will drift farther down which is why we need ReachTargetThreshold
		[SerializeField] private float m_ReachTargetThreshold;
		// proximity to target to consider we 'reached' it, and stop driving.
		[SerializeField] public WaypointCircuit circuit;
		...
		//Plays audio for switching from autonomous to manual when "A" is pressed on ocolus controller
   ...
   //             GetComponent<CarAIControl>().enabled = false;
            ...
            if (m_Target == null || !m_Driving) {
                ...
                else if(TooFar)
                {
                    ...
                    s.Log("Too Far Accel : " + accel + "  MaxSpeed: " + m_CarController.MaxSpeed);
                }
                */
				...
				if (m_IsTargetReached && localTarget.magnitude < m_ReachTargetThreshold) {
					m_Driving = false;
				} else if (!m_IsTargetReached && localTarget.magnitude < m_ReachTargetThreshold) {
					if (m_isCircuit) {
		...
		public void SetTarget (Transform target, bool IsTargetReached)
		{
			m_Target = target;
			m_Driving = true;
			m_IsTargetReached = IsTargetReached;
